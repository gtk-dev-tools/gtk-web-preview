...
const postcssGTKPlugin = require('postcss').plugin('postcss-gtk-plugin', options => (r) => {
    const css = r.source.input.css;
    let cssString = '';
    let overrides = [
      [/[^:]hover/g, 'a:hover'],
      [/\.button/g, 'button'],
    ...
    for (let i = 0; i < overrides.length; i++) {
      cssString = css.replace(...overrides[i]);
    }

    return require('postcss-gtk').process(cssString);
  }
);
